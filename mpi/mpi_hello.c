...
int main(void) {
   ...
   MPI_Get_processor_name(my_server, &server_length);

   if (my_rank != 0) {
      /* Create message */
      sprintf(greeting, "Greetings at %s from process %d of %d!", my_server,
            my_rank, comm_sz);
      ...
      /* Print my message */
      printf("Greetings at %s from process %d of %d!", my_server,
            my_rank, comm_sz);
